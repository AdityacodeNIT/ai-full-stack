...
import ProctoringManager from '../proctoring/ProctoringManager.jsx';
...
const InterviewSession = ({ interviewId }) => {
  ...
  const { lockInterview, unlockInterview,isInterviewActive } = useInterviewLock();
  ...
  const submitResponse = useCallback((sendFn) => {
    ...
    if (currentTranscript.length > 5 && !hasSubmittedResponse.current) {
      ...
      log('Submitting response:', currentTranscript);
      sendFn({
        type: 'response',
  ...
  const handlers = useMemo(() => ({
    ...
    acknowledgment: (msg) => {
      ...
      setIsProcessingResponse(false);
      log(' Answer recorded:', msg.message);
    ...
    evaluating: (msg) => {
      log(' Evaluating interview:', msg.message);
      setIsProcessingResponse(true);
    },
  ...
  // Lock interview when session starts
  useEffect(() => {
    if (status === 'connected' && !completed) {
      lockInterview(interviewId);

      // Request fullscreen
      if (document.documentElement.requestFullscreen) {
        document.documentElement.requestFullscreen().catch(err => {
          warn('Fullscreen request failed:', err);
        });
      }
    }

    // Unlock when completed or on unmount
    return () => {
      if (status === 'error') {
        unlockInterview();
        ...
        if (document.fullscreenElement) {
          document.exitFullscreen().catch(err => {
            warn('Exit fullscreen failed:', err);
          });
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      {/* Interview Lock Indicator */}
      {isInterviewActive && (
        <div className="fixed top-0 left-0 right-0 bg-red-900 text-white text-center py-2 z-50">
          <span className="font-semibold"> INTERVIEW IN PROGRESS - Navigation Locked</span>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto mt-12">
        {status === 'connected' && !completed && startProctoring && (
          <ProctoringManager 
            key="proctoring-manager" 
            interviewWS={interviewWS.current} 
            videoElement={userVideoRef.current}
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            <div
              className={`w-3 h-3 mr-2 rounded-full ${status === 'connected'
                  ? 'bg-green-500'
                  : status === 'error'
                    ? 'bg-red-500'
                    : 'bg-gray-500'
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-3 h-3 rounded-full ${isTranscribing ? 'bg-red-500 animate-pulse' : 'bg-gray-600'
                              }`}
                          />
                  ...
                  <div className="bg-gradient-to-br from-blue-900 to-purple-900 rounded-lg overflow-hidden shadow-lg h-80 flex items-center justify-center">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-3 h-3 rounded-full ${isSpeaking ? 'bg-blue-500 animate-pulse' : 'bg-gray-600'
                              }`}
                          />
                          <span className="text-sm text-white">
        ...
        {finalReport && (
          <div className="mt-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-blue-900 to-purple-900 p-6 rounded-lg shadow-lg">
              ...
              <div className="mb-4">
                <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${finalReport.recommendation === 'Strong Hire' ? 'bg-green-500 text-white' :
                    ...
                    <div key={index} className="bg-gray-700 p-4 rounded-lg">
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-3 text-xs">
                        <div className="bg-gray-600 p-2 rounded text-center">
                          ...
                          <div className={`font-semibold ${evalData.confidence === 'High' ? 'text-green-400' :
                        ...
                        <div className="bg-gray-600 p-2 rounded text-center">
                          ...
                          <div className={`font-semibold ${evalData.clarity === 'High' ? 'text-green-400' :
                        ...
                        <div className="bg-gray-600 p-2 rounded text-center">
                          ...
                          <div className={`font-semibold ${evalData.technicalUnderstanding === 'High' ? 'text-green-400' :
