...
export class InterviewHandler {
  ...
  async handleProctoringViolation(data) {
    try {
      ...
      // Store in session (will be saved to DB on interview completion)
      if (!this.session.proctoringViolations) {
        this.session.proctoringViolations = [];
      }
      this.session.proctoringViolations.push(violation);
      ...
      if (violation.metadata.object) {
        const confidence = violation.metadata.confidence 
          ...
          : 'N/A';
        logger.log(`🚨 [${violation.detector}] ${violation.type}: ${violation.metadata.object} (${confidence} confidence) - Total violations: ${this.session.proctoringViolations.length}`);
      } else {
        logger.log(`🚨 [${violation.detector}] ${violation.type} - Total violations: ${this.session.proctoringViolations.length}`);
      }
    } catch (err) {
      logger.error("❌ handleProctoringViolation failed:", err);
      // Don't throw - just log the error so interview continues
    }
  }
  ...
  async completeInterview() {
    try {
      ...
      if (this.session.proctoringViolations && this.session.proctoringViolations.length > 0) {
        logger.log(`📊 Proctoring Summary: ${this.session.proctoringViolations.length} violation(s) recorded`);
      }
